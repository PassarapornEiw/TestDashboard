...
try:
    ...
    print("Warning: Matplotlib not installed. Charts in PDF will be disabled.")
...
try:
    ...
    print("Warning: Playwright not available. HTML thumbnail generation disabled.")
...
# Automation Project directory - configurable via environment variable
AUTOMATION_PROJECT_DIR = Path(os.environ.get('AUTOMATION_PROJECT_DIR', PROJECT_ROOT / "Automation_Project"))
...
def discover_results_directory():
    ...
    if env_results_dir:
        ...
        if env_path.exists() and env_path.is_dir():
            ...
            print(f"Warning: Environment RESULTS_DIR not found: {env_path}")
    ...
    print(f"Warning: No results directory found. Creating default: {default_results}")
...
class EvidenceProcessor:
    """Centralized evidence file processing for consistent behavior across Gallery, PDF, and ZIP exports."""
    
    MEDIA_EXTENSIONS = {'.png', '.jpg', '.jpeg', '.gif', '.bmp', '.html', '.htm'}
    EXCEL_EXTENSIONS = {'.xlsx', '.xls'}
    
    @staticmethod
    def extract_sort_key(file_path: str) -> tuple:
        """Extract sort key for evidence file.
        
        Returns tuple (is_excel: bool, number: int, filename: str) for sorting.
        ...
        Returns:
            Tuple (is_excel, number, filename) for sorting
        """
        import re
        from pathlib import Path
    ...
    @staticmethod
    def ensure_thumbnail_exists(file_path: Path) -> Path:
        """Ensure thumbnail exists for HTML file, generate if missing.
        
        For HTML files, checks if thumbnail exists and generates it if missing.
        For other file types, returns the original path.
        
        Args:
            file_path: Absolute path to evidence file
            
        Returns:
            Path to thumbnail (for HTML) or original file path (for others)
        """
        ...
        if thumb_path.exists():
            # Check if thumbnail is newer than HTML file (or HTML hasn't changed)
            html_mtime = file_path.stat().st_mtime
            thumb_mtime = thumb_path.stat().st_mtime
            
            if thumb_mtime >= html_mtime:
                # Thumbnail exists and is up-to-date
                return thumb_path
        
        # Generate thumbnail if missing or outdated
        print(f"[INFO] Generating thumbnail for HTML file: {file_path}")
        try:
            thumbnail_success = _html_to_thumbnail(file_path, thumb_path)
            if thumbnail_success and thumb_path.exists():
                return thumb_path
            else:
                print(f"[WARN] Thumbnail generation failed for {file_path}, using original")
                return file_path
        except Exception as e:
            print(f"[ERROR] Error generating thumbnail for {file_path}: {e}")
            return file_path
    
    @staticmethod
    def prepare_evidence_for_pdf(evidence_list: list, project_root: Path = None) -> list:
        """Prepare evidence list with thumbnails ready for PDF generation.
        
        This method:
        1. Sorts evidence using centralized sorting logic
        2. Ensures HTML thumbnails exist (generates if needed)
        3. Returns list ready for PDF processing
        ...
        Returns:
            List of evidence file paths (sorted, with thumbnails ready)
        """
...
def verify_font_settings():
    ...
    if REPORTLAB_AVAILABLE:
        ...
        try:
            ...
            print(f"Error: Font not properly registered: {e}")
    ...
    if is_thai:
        ...
        print("Error: Still using default Helvetica (no Thai support)")
...
def ensure_thai_fonts():
    ...
    try:
        ...
        print(f"Warning: Could not register CID fonts: {e}")

    # If nothing works, warn and keep Helvetica (no Thai support)
    print("[WARNING] Warning: No Thai fonts found! Using Helvetica (no Thai support)")
    return False
    ...
    try:
        ...
        if result_folder_col and test_case_col:
            ...
            for idx, row in df.iterrows():
                ...
                if evidence_files:
                    # Convert to relative paths from PROJECT_ROOT (not RESULTS_DIR.parent)
                    relative_paths = []
                        ...
                        try:
                            if PROJECT_ROOT in evidence_path.parents:
                                ...
                                if relative_evidence_path:
                                    relative_paths.append(relative_evidence_path)
                            else:
...
def discover_projects():
    """
    Discover all projects in AUTOMATION_PROJECT_DIR that have a 'results' folder.
    ...
    if not AUTOMATION_PROJECT_DIR.exists() or not AUTOMATION_PROJECT_DIR.is_dir():
        print(f"Warning: Automation Project directory not found: {AUTOMATION_PROJECT_DIR}")
        return projects
    
    print(f"🔍 Discovering projects in: {AUTOMATION_PROJECT_DIR}")
    
    # Iterate through subdirectories in AUTOMATION_PROJECT_DIR
    for item in AUTOMATION_PROJECT_DIR.iterdir():
        if item.is_dir() and not item.name.startswith('.'):
            results_folder = item / "results"
            if results_folder.exists() and results_folder.is_dir():
                # Check if results folder contains valid timestamp folders
                timestamp_dirs = [d for d in results_folder.iterdir() 
                                if d.is_dir() and is_valid_timestamp_folder(d.name)]
                ...
                if timestamp_dirs:
                    ...
                    print(f" Found project: {item.name} (with {len(timestamp_dirs)} timestamp folders)")
                else:
                    print(f" Found project {item.name} but no valid timestamp data")
    
    return sorted(projects, key=lambda x: x["name"])

def get_project_stats(project_path):
    """
    Calculate statistics for a project from its latest run.
    
    Args:
        project_path: Relative path to project from PROJECT_ROOT (e.g., "Automation_Project/LDP_Test_Automation")
    
    Returns:
        Dictionary with stats: pass_rate, total_tests, passed, failed, last_run
    """
    global RESULTS_DIR
    original_results_dir = RESULTS_DIR
    
    try:
        # Build full path to project's results folder
        project_full_path = PROJECT_ROOT / project_path
        results_dir = project_full_path / "results"
        ...
        # Temporarily set RESULTS_DIR to this project's results folder
        RESULTS_DIR = results_dir
        
        # Find all Excel files in this project's results
        ...
        # Aggregate stats from all features in the latest timestamp folder
        # Group by timestamp to get the latest run
        runs = {}
        for excel_file in all_excel_files:
            ...
            if feature_data and feature_data.get("total", 0) > 0:
                ...
                runs[timestamp]["total"] += feature_data.get("total", 0)
                runs[timestamp]["passed"] += feature_data.get("passed", 0)
                runs[timestamp]["failed"] += feature_data.get("failed", 0)
        ...
        # Get the latest run (most recent timestamp)
        ...
        print(f"Error calculating stats for project {project_path}: {e}")
        RESULTS_DIR = original_results_dir
        return {
            "pass_rate": 0,
            "total_tests": 0,
...
def get_projects():
    """Get list of all available projects with their statistics."""
    try:
        ...
        for project in projects:
            stats = get_project_stats(project["path"])
        ...
        return jsonify({
            ...
            "automation_project_dir": str(AUTOMATION_PROJECT_DIR)
        })
    except Exception as e:
...
def get_dashboard_data():
    """
    Get dashboard data for a specific project or use auto-discovered results.
    
    Query parameters:
        project: (optional) Project name to load data from. If not provided, uses auto-discovered RESULTS_DIR.
    """
    global RESULTS_DIR
    original_results_dir = RESULTS_DIR
    ...
    try:
        ...
        if project_name:
            ...
            # Build path to project's results folder
            project_full_path = PROJECT_ROOT / project["path"]
            project_results_dir = project_full_path / "results"
            
            if not project_results_dir.exists() or not project_results_dir.is_dir():
                RESULTS_DIR = original_results_dir
                return jsonify({"error": f"Results folder not found for project '{project_name}'"}), 404
            
            # Temporarily set RESULTS_DIR to this project's results folder
            RESULTS_DIR = project_results_dir
            print(f" Loading data for project: {project_name} from {RESULTS_DIR}")
        else:
            # Backward compatibility: use original auto-discovered RESULTS_DIR
            print(f" Loading data from auto-discovered results: {RESULTS_DIR}")
...
def excel_preview():
    ...
    try:
        ...
        # Path is now relative to PROJECT_ROOT (changed from RESULTS_DIR.parent)
        rel_norm = excel_path.lstrip('/\\')
        full_path = (PROJECT_ROOT / rel_norm)
...
def export_pdf():
    ...
    try:
        ...
        content_width = A4[0] - doc.leftMargin - doc.rightMargin  # Define content width for tables
                    ...
                    if df is not None and not df.empty:
                        ...
                        if id_col and status_col:
                            ...
                            for _, row in df.iterrows():
                                    ...
                                    try:
                                        ...
                                        if error_col:
                                            ...
                                            if error_msg and error_msg.lower() not in ['nan', 'none', '']:
                                                ...
                                                elements.append(Paragraph(f"<b><font color='#B71C1C'>Error: Failure Reason:</font></b><br/><font color='#D32F2F'>{error_msg}</font>", error_style))
                                                elements.append(Spacer(1, 8))
                                    except Exception as e:
                                ...
                                if images:
                                    # Use EvidenceProcessor to ensure proper sorting and thumbnail preparation
                                    # Images are already sorted from parse_excel_data, but ensure thumbnails are ready
                                    all_images = EvidenceProcessor.prepare_evidence_for_pdf(images, PROJECT_ROOT)
                                            ...
                                            if img_abs.exists():
                                                try:
                                                    ...
                                                    if file_extension in ['xlsx', 'xls']:
                                                        # Generate Excel placeholder image
                                                        try:
                                                            if PIL_AVAILABLE:
                                                                excel_img = PILImage.new("RGB", (500, 400), color=(33, 115, 70))
                                                                ...
                                                                draw.rectangle([50, 50, 450, 350], fill=(255, 255, 255), outline=(0, 0, 0), width=2)
                                                                draw.text((250, 150), "📊", fill=(0, 0, 0), font=font, anchor="mm")
                                                                draw.text((250, 200), "Excel File", fill=(0, 0, 0), font=font, anchor="mm")
                                                                draw.text((250, 230), img_filename[:40], fill=(64, 64, 64), font=font, anchor="mm")
                                                                
                                                                with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as temp_img:
                                                                    excel_img.save(temp_img.name, format='JPEG', quality=85)
                                                                    ...
                                                                    file_type_label = "Excel File"
                                                                    img_with_caption = [
                                                                        [reportlab_img],
                                                                        [Paragraph(f"<b>{file_type_label}:</b> {img_filename}", caption_style)]
                                                                    ]
                                                                    evidence_table = Table(img_with_caption, colWidths=[content_width])
                                                                    ...
                                                                    elements.append(Spacer(1, 10))
                                                                    print(f"[DEBUG] Added Excel file to PDF: {img_filename}")
                                                            else:
                                                                elements.append(Paragraph(f"<b>Excel File:</b> {img_filename}", caption_style))
                                                                elements.append(Spacer(1, 10))
                                                        except Exception as e:
                                                            print(f"[ERROR] Error processing Excel file {img_path}: {e}")
                                                            elements.append(Paragraph(f"<b>Excel File:</b> {img_filename} (Error processing)", caption_style))
                                                            elements.append(Spacer(1, 10))
                                                        continue
                                    
                                                    # Check if this is an HTML file
                                                    if file_extension in ['html', 'htm']:
                                                        # Use EvidenceProcessor to ensure thumbnail exists
                                                        print(f"[DEBUG] Ensuring thumbnail exists for HTML: {img_path}")
                                                        thumbnail_path = EvidenceProcessor.ensure_thumbnail_exists(img_abs)
                                                        
                                                        if thumbnail_path.exists() and thumbnail_path.suffix.lower() == '.png':
                                                            print(f"[DEBUG] Using thumbnail: {thumbnail_path}")
                                                            ...
                                                            print(f"[DEBUG] Using thumbnail. Dimensions: {pdf_width}x{pdf_height}")
                                                        else:
                                                            print(f"[DEBUG] No thumbnail available, creating placeholder")
                                                            reportlab_img = None
                                                        
                                                        if not reportlab_img:
...
def generate_test_case_pdf_core(test_case_id, feature_name, run_timestamp, feature_data, test_case_row, mode='full'):
    ...
    try:
        ...
        if screenshots:
            ...
            # Use centralized EvidenceProcessor to sort evidence (media first, Excel last)
            all_evidence = EvidenceProcessor.prepare_evidence_for_pdf(screenshots, PROJECT_ROOT)
            ...
            if all_evidence:
                ...
                for idx, file_path in enumerate(all_evidence, 1):
                    ...
                    print(f"[DEBUG] Processing evidence file: {file_abs}")
                    ...
                    # Handle Excel files - show as SVG placeholder
                    if file_extension in ['xls', 'xlsx']:
                        try:
                            # Generate SVG placeholder for Excel file
                            svg_bytes = _excel_preview_placeholder_svg(filename, width=800, height=450)
                            
                            # Convert SVG to PNG for ReportLab (using PIL if available)
                            if PIL_AVAILABLE:
                                from io import BytesIO
                                import xml.etree.ElementTree as ET
                                
                                # Parse SVG and create PIL image
                                # For simplicity, create a colored rectangle with text
                                excel_img = PILImage.new("RGB", (400, 300), color=(33, 115, 70))  # Excel green
                                draw = ImageDraw.Draw(excel_img)
                                ...
                                # Draw Excel icon representation
                                draw.rectangle([50, 50, 350, 250], fill=(255, 255, 255), outline=(0, 0, 0), width=2)
                                draw.text((200, 100), "📊", fill=(0, 0, 0), font=font, anchor="mm")
                                draw.text((200, 150), "Excel File", fill=(0, 0, 0), font=font, anchor="mm")
                                draw.text((200, 180), filename[:30], fill=(64, 64, 64), font=font, anchor="mm")
                                ...
                                with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as temp_img:
                                    ...
                                    elements.append(evidence_table)
                                    elements.append(Spacer(1, 10))
                                    print(f"[DEBUG] Added Excel file to PDF: {filename}")
                            else:
                                # If PIL not available, show as text
                                elements.append(Paragraph(f"<b>Excel File:</b> {filename}", caption_style))
                                elements.append(Spacer(1, 10))
                        except Exception as e:
                            print(f"[ERROR] Failed to create Excel placeholder image: {e}")
                            # Fallback: just show filename as text
                            elements.append(Paragraph(f"<b>Excel File:</b> {filename} (Preview not available)", caption_style))
                            elements.append(Spacer(1, 10))
                        continue
                    if file_abs.exists():
                        try:
                            ...
                            if file_extension in ['html', 'htm']:
                                # Use EvidenceProcessor to ensure thumbnail exists
                                print(f"[DEBUG] Ensuring thumbnail exists for HTML file: {file_path}")
                                thumbnail_path = EvidenceProcessor.ensure_thumbnail_exists(file_abs)
                                
                                if thumbnail_path.exists() and thumbnail_path.suffix.lower() == '.png':
                                    print(f"[DEBUG] Using thumbnail for PDF: {thumbnail_path}")
                                ...
                                if reportlab_img:
                                    print(f"[DEBUG] Using thumbnail for PDF. Dimensions: {pdf_width}x{pdf_height}")
                                else:
...
def export_testcase_pdf():
    ...
    try:
        ...
        # Set project-specific RESULTS_DIR if project is provided
        if project_name:
            projects = discover_projects()
            project = next((p for p in projects if p["name"] == project_name), None)
            if project:
                ...
                if project_results_dir.exists() and project_results_dir.is_dir():
                    RESULTS_DIR = project_results_dir
                    print(f"[DEBUG] Using project-specific RESULTS_DIR: {RESULTS_DIR}")
                else:
                    print(f"[WARN] Project results folder not found: {project_results_dir}, using default")
            else:
                print(f"[WARN] Project '{project_name}' not found, using default RESULTS_DIR")
...
def serve_results(filepath):
    ...
    Path is relative to PROJECT_ROOT (e.g., 'Automation_Project/DRDB/results/...')
    """
    # Path is relative to PROJECT_ROOT, not RESULTS_DIR
    # Try to resolve from PROJECT_ROOT first
...
def _get_thumbnail_path(html_path: Path) -> Path:
    """Generate a thumbnail path in the same directory as the HTML file.
    
    Thumbnails are stored directly in the test case folder with _preview suffix.
    Example: testresult.html -> testresult_preview.png
    """
    # Simple and clean: store thumbnail in same directory as HTML file
    filename = f"{html_path.stem}_preview.png"
    return html_path.parent / filename
...
def _html_to_thumbnail(html_abs_path: Path, thumb_abs_path: Path = None, width: int = 800, height: int = 450):
    ...
    # Note: thumb_abs_path.parent.mkdir() is not needed since thumbnail is stored
    # directly in the test case folder which should already exist
...
def api_evidence_thumbnail():
    ...
    try:
        ...
        if PROJECT_ROOT not in abs_path.parents and abs_path != PROJECT_ROOT:
            return abort(400, description="Path must be under project root")
    ...
    if file_ext in ['.html', '.htm']:
        ...
        try:
            ...
            if need_build:
                ...
                print(f"[INFO] Thumbnail will be saved to: {thumb_path}")
...
def get_thumbnail_info():
    ...
    NEW STRUCTURE: results/YYYYMMDD_HHMMSS/FeatureName/TestCaseID/{filename}_preview.png
    Thumbnails are stored directly in test case folders alongside other evidence files.
    """
    try:
        total_files = 0
        total_size = 0
        ...
        # Scan for *_preview.png files in test case folders
        for thumb_file in RESULTS_DIR.rglob("*_preview.png"):
            try:
                if thumb_file.is_file():
                    total_files += 1
                    total_size += thumb_file.stat().st_size
                    
                    # Extract folder path for reporting
                    # Structure: results/YYYYMMDD_HHMMSS/FeatureName/TestCaseID/file_preview.png
                    parent = thumb_file.parent
                    if parent.parent and parent.parent.parent:
                        timestamp_folder = parent.parent.parent
                        if timestamp_folder.name and any(char.isdigit() for char in timestamp_folder.name):
                            folder_info = f"{timestamp_folder.name}/{parent.parent.name}/{parent.name}"
                            folders.add(folder_info)
            except Exception as e:
                print(f"[WARN] Error scanning thumbnail file {thumb_file}: {e}")
        ...
        return {
            ...
            "structure": "NEW: {filename}_preview.png in TestCaseID folder"
        }
    except Exception as e:
...
def api_thumbnail_status():
    ...
    # Check for old .thumbnails folders that need cleanup
    old_thumbnails_dirs = []
    old_files_count = 0
    
    # Scan for .thumbnails directories in results
    for thumb_dir in RESULTS_DIR.rglob(".thumbnails"):
        if thumb_dir.is_dir():
            old_thumbnails_dirs.append(str(thumb_dir.relative_to(RESULTS_DIR)))
            # Count files in old .thumbnails folder
            try:
    ...
    needs_cleanup = len(old_thumbnails_dirs) > 0
    
    return jsonify({
        "cache_info": cache_info,
        "old_structure": {
            "old_directories": len(old_thumbnails_dirs),
            "old_files": old_files_count,
            "needs_cleanup": needs_cleanup,
            "old_directories_list": old_thumbnails_dirs[:10]  # Limit to first 10 for response size
        },
        "recommendations": {
            "cleanup_needed": needs_cleanup,
            "message": f"Found {len(old_thumbnails_dirs)} old .thumbnails directories with {old_files_count} files that can be cleaned up" if needs_cleanup else "No cleanup needed"
        }
    })

@app.route('/api/cleanup_old_thumbnails', methods=['POST'])
def api_cleanup_old_thumbnails():
    """Remove old .thumbnails directories from the results folder."""
    try:
        removed_dirs = []
        removed_files = 0
        errors = []
        ...
        for thumb_dir in RESULTS_DIR.rglob(".thumbnails"):
            if thumb_dir.is_dir():
                try:
                    ...
                    file_count = sum(1 for item in thumb_dir.iterdir() if item.is_file())
                    
                    # Remove the entire .thumbnails directory
                    shutil.rmtree(thumb_dir)
                    
                    removed_dirs.append(str(thumb_dir.relative_to(RESULTS_DIR)))
                    removed_files += file_count
                except Exception as e:
                    errors.append({
                        "directory": str(thumb_dir.relative_to(RESULTS_DIR)),
                        "error": str(e)
                    })
        ...
        return jsonify({
            ...
            "removed_directories_list": removed_dirs,
...
if __name__ == "__main__":
    ...
    if RESULTS_DIR.exists():
        ...
        if timestamp_dirs:
            ...
            print("Warning: No test run data found in results directory")
            print("   Place test results in YYYYMMDD-HHMMSS format folders")
    else:
        print(f"Error: Results directory not accessible: {RESULTS_DIR}")
    ...
    if THUMBNAIL_CAPABLE:
        ...
        print("Error: Overall: No thumbnail generation capability")
        print("   Install either Playwright or PIL/Pillow")
    
    print("="*60)
    
    # Thumbnail system info
    if THUMBNAIL_CAPABLE:
        print(f"📸 Thumbnail system ready")
        print(f" Thumbnails are stored directly in test case folders")
        print(f"   STRUCTURE: results/YYYYMMDD_HHMMSS/FeatureName/TestCaseID/{{filename}}_preview.png")
        print(f"   Example: testresult.html -> testresult_preview.png")
        
        # Test thumbnail path structure
        try:
            test_html_path = RESULTS_DIR / "test_example.html"
            test_thumb_path = _get_thumbnail_path(test_html_path)
            print(f"🔍 Test thumbnail path: {test_thumb_path}")
            print(f"   Thumbnails stored alongside evidence files in test case folders")
        except Exception as e:
            print(f"Warning: Could not demonstrate thumbnail structure: {e}")
    ...
    # Start the Flask app - use use_reloader=False for subprocess execution
