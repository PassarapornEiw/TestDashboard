...
## 8. Evidence Processing และ Performance Optimization

### 8.1 EvidenceProcessor Class (Backend)
```python
class EvidenceProcessor:
    """Centralized evidence file processing for consistent behavior."""
    ...
    def extract_sort_key(file_path: str) -> tuple:
        """Extract sort key: (is_excel, number, filename)"""
        # Excel files go last (True sorts after False)
        # Media files sorted by leading number
        
    @staticmethod
    def collect_and_sort_evidence(evidence_list: list) -> list:
        """Sort evidence: media by number first, Excel last by name."""
        # Returns sorted list with consistent ordering
        
    @staticmethod
    def ensure_thumbnail_exists(file_path: Path) -> Path:
        """Generate thumbnail for HTML if missing, return thumbnail path."""
        # Legacy support - HTML files are not collected as evidence
        # Checks if thumbnail exists and is up-to-date
        # Generates if needed using Playwright or PIL fallback
        
    @staticmethod
    def prepare_evidence_for_pdf(evidence_list: list, project_root: Path) -> list:
        """Prepare evidence with thumbnails ready for PDF generation."""
        # Sorts evidence (HTML files are not in evidence list)
```

### 8.2 Centralized Sorting Logic
```python
# Sorting behavior:
# 1. Media files (images) sorted by leading number in filename
# 2. Excel files sorted alphabetically, always at the end

# Example sorting result:
# - 1_screenshot.png
# - 2_evidence.png
# - 3_result.png
# - data.xlsx (always last)
# - testdata.xlsx (always last)

# Note: HTML files are not collected as evidence (screenshots exist as PNG)
```

### 8.3 Thumbnail Caching
```javascript
...
- **Single Sort**: Evidence files sorted once during collection, not at display time
- **No HTML Processing**: HTML files excluded from evidence (screenshots exist as PNG)
- **Consistent Ordering**: Same sorting logic used everywhere (Gallery, PDF, ZIP)
...
## 12. Evidence Processing Architecture

### 12.1 EvidenceProcessor Integration
```python
# Usage in parse_excel_data()
test_evidence[test_case_id] = EvidenceProcessor.collect_and_sort_evidence(relative_paths)

# Usage in PDF generation
sorted_evidence = EvidenceProcessor.prepare_evidence_for_pdf(screenshots, PROJECT_ROOT)

# Usage in Gallery (frontend receives pre-sorted data)
# No sorting needed - data already sorted by backend
```

### 12.2 Excel Files Support
- **Gallery**: Excel files displayed as thumbnail icons (click to download)
- **PDF**: Excel files shown as placeholder images with filename
- **Sorting**: Excel files always appear at the end of evidence list

### 12.3 Thumbnail Management
- **Excel Files**: Placeholder thumbnails (SVG) generated on-demand
- **Image Files**: Direct display (no thumbnail generation needed)
- **HTML Files**: Excluded from evidence (screenshots exist as PNG from automation)
- **Note**: HTML thumbnail generation code remains for legacy support but is not used
...
6. **EvidenceProcessor Class** สำหรับ centralized evidence processing
7. **Consistent Sorting Logic** ที่ใช้ใน Gallery, PDF, และ ZIP exports
8. **Excel Files Support** ใน Gallery และ PDF
9. **Optimized Performance** ด้วย single sort และ thumbnail reuse

ระบบมีความเสถียรและรองรับการขยายตัวในอนาคต โดยมีการจัดการ priority, สี, และ evidence processing ที่ชัดเจนและสอดคล้องกัน
