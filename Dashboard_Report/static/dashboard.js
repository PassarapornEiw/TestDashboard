...
async function loadDashboardData() {
    try {
        // Get project parameter from URL query string
        const urlParams = new URLSearchParams(window.location.search);
        const project = urlParams.get('project');
        
        // Build API URL with project parameter if present
        let apiUrl = '/api/data';
        if (project) {
            apiUrl += '?project=' + encodeURIComponent(project);
        }
...
async function generateTestCaseGallery(feature, testCaseDetails, galleryId) {
    ...
    for (const [excelTestCaseId, testCaseDetail] of Object.entries(testCaseDetails)) {
        ...
        if (matchingImages && matchingImages.length > 0) {
            ...
            // Data is already sorted by backend EvidenceProcessor (media files by number, Excel files last)
            // No need to sort again here - use pre-sorted data directly
            ...
            if (validImages.length > 0) {
                ...
                previewImages.forEach(imgPath => {
                    ...
                    if (!imgPath.startsWith('/results/')) {
                        // Strip optional 'Automation Project/' prefix if present (with space)
                        if (imgPath.startsWith('Automation Project/')) {
                            fixedPath = imgPath.replace(/^Automation Project[\\/]/, '');
                        } else {
                            fixedPath = imgPath;
                        }
                        // Use /results/ prefix for all paths (backend resolves from PROJECT_ROOT)
                        if (fixedPath.startsWith('results/') || fixedPath.startsWith('Automation_Project/')) {
                    ...
                    // For API, use path relative to PROJECT_ROOT (remove /results/ prefix)
                    const relForApi = fixedPath.startsWith('/results/') ? fixedPath.slice(9) : fixedPath.replace(/^\//, '');
...
async function openAllImagesFromData(element) {
    try {
        ...
        images.forEach((imgPath, index) => {
            ...
            // Ensure proper /results/ prefix (backend resolves from PROJECT_ROOT)
        ...
        // Eagerly generate thumbnails for HTML files to ensure they're ready for display
...
// Helper function to eagerly generate thumbnails for evidence files (HTML, Excel, Images)
async function prefetchEvidenceThumbnails(imagePaths) {
    try {
        console.log('[DEBUG] Prefetching thumbnails for', imagePaths.length, 'files');
...
function showAllImagesModal(testCaseName, images, status, actualFolderName = null) {
    ...
    const statusBadge = status === 'pass' ? '<span class="status-badge status-passed">PASS</span>' :
                         ...
                         status === 'fail' ? '<span class="status-badge status-failed-major">FAIL (Major)</span>' :
    ...
    const galleryHTML = images.map((rawPath, index) => {
        ...
        if (!rawPath.startsWith('/results/')) {
            ...
            // Use /results/ prefix for all paths (backend resolves from PROJECT_ROOT)
            if (stripped.startsWith('results/') || stripped.startsWith('Automation_Project/')) {
                fixedPath = '/results/' + stripped;
            } else {
                fixedPath = '/results/' + stripped.replace(/^\//, '');
            }
        }
