...
# --- Multi-URL demo (URLs จาก LDP_UI.yaml) ---

Open Google And Search
    [Documentation]    เปิด Google (URL จาก config) และค้นหาตามคำที่กำหนด
    [Arguments]    ${search_term}
    ${google_url}=    ConfigReader.Get Multi Url Demo Google Url
    Go To    ${google_url}
    Wait Until Page Contains Element    ${GOOGLE_SEARCH_INPUT}    timeout=15s
    Input Text    ${GOOGLE_SEARCH_INPUT}    ${search_term}
    Press Keys    ${GOOGLE_SEARCH_INPUT}    RETURN
    Sleep    1s

Open Facebook Tab And Fill Login
    [Documentation]    เปิด tab ใหม่ไป Facebook (URL จาก config) กรอก username/password (ไม่กด login)
    [Arguments]    ${username}    ${password}
    ${facebook_url}=    ConfigReader.Get Multi Url Demo Facebook Url
    Execute JavaScript    window.open('${facebook_url}', '_blank');
    Switch Window    NEW
    Wait Until Page Contains Element    ${FACEBOOK_EMAIL_INPUT}    timeout=15s
    Input Text    ${FACEBOOK_EMAIL_INPUT}    ${username}
    Input Password    ${FACEBOOK_PASS_INPUT}    ${password}

Close Current Tab And Switch Back
    [Documentation]    ปิด tab ปัจจุบัน แล้วสลับกลับไป tab ก่อนหน้า
    Close Window
    Switch Window    MAIN

Open YouTube In New Tab
    [Documentation]    เปิด tab ใหม่ไป YouTube (URL จาก config)
    ${youtube_url}=    ConfigReader.Get Multi Url Demo Youtube Url
    Execute JavaScript    window.open('${youtube_url}', '_blank');
    Switch Window    NEW
    Wait Until Page Contains    YouTube    timeout=15s

Run Multi URL Demo Flow
    [Documentation]    รัน flow: Google (test system) -> Facebook tab (fill จาก config, close) -> Google (เทสระบบ) -> YouTube tab
    ${creds}=    ConfigReader.Get Multi Url Demo Facebook Credentials
    Open Google And Search    test system
    Open Facebook Tab And Fill Login    ${creds}[username]    ${creds}[password]
    Close Current Tab And Switch Back
    Open Google And Search    เทสระบบ
    Open YouTube In New Tab
